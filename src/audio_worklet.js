...
const zig = {
  ...
  zigCreateString(str, len) {
    let memory = new MemoryBlock(zig.memory.buffer);
  ...
  getProperty(prop, ret_ptr) {
    return zig.getPropertyEx(prop, zig.memory.buffer, ret_ptr);
  },
...
registerProcessor("WasmProcessor", class WasmProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this.instance = new WebAssembly.Instance(options.processorOptions.module, { zig, env: { memory: this.memory } });
  }
  
  process(inputs, outputs) {
    const result_buffer = this.instance.exports.process(currentTime, sampleRate, outputs[0][0].length);
    if (result_buffer !== 0) {
      outputs[0][0].set(new Float32Array(this.memory.buffer, result_buffer, outputs[0][0].length));
    }
    return true;
  }
});
